...
export const getOpportunity = asyncHandler(async (req, res) => {
    ...
    const resume = await ResumeParsed.findOne({ user: userId }).sort({ createdAt: -1 });
    ...
    if (!resume || !resume.categories || resume.categories.length === 0) {
        ...
        return res.status(200).json(
            new apiResponse(200, "Upload resume for personalized results", opportunities)
        );
    }
    ...
    return res.status(200).json(
        new apiResponse(200, `Opportunities for ${categoryNames.join(", ")} fetched`, matchedOpportunities)
    );
});