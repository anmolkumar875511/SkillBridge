...
const Logger = () => {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
            ...
            <p className="text-xs font-bold uppercase tracking-widest opacity-50" style={{ color: theme.colors.textMuted }}>Retrieving Logs...</p>
        </div>
    );
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b" style={{ borderColor: theme.colors.border }}>
                                ...
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Severity</th>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Action</th>
                                <th className="px-6 py-5 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Initiator</th>
                            ...
                            {logs.length > 0 ? logs.map((log) => (
                                <tr key={log._id} className="hover:bg-slate-50 transition-colors group">
                                    <td className="px-8 py-4 whitespace-nowrap">
                                        <p className="text-xs font-bold" style={{ color: theme.colors.textMain }}>{new Date(log.createdAt).toLocaleDateString()}</p>
                                        <p className="text-[10px] font-medium opacity-50" style={{ color: theme.colors.textMuted }}>{new Date(log.createdAt).toLocaleTimeString()}</p>
                                    </td>
                                    <td className="px-6 py-4">
                                        <LogLevelBadge level={log.level} />
                                    </td>
                                    <td className="px-6 py-4 text-[11px] font-bold uppercase tracking-wide" style={{ color: theme.colors.primary }}>
                                        {log.meta?.action || 'SYSTEM_EVENT'}
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-bold border" style={{ backgroundColor: theme.colors.bgLight, color: theme.colors.textMuted, borderColor: theme.colors.border }}>
                                    ...
                                    <td className="px-8 py-4 text-xs font-medium max-w-xs xl:max-w-md truncate group-hover:whitespace-normal" style={{ color: theme.colors.textMuted }}>
                                        {log.message}
                                    </td>
                                </tr>
                            )) : (
                                <tr>
                                    <td colSpan="5" className="px-8 py-20 text-center">
                                        <div className="flex flex-col items-center opacity-20" style={{ color: theme.colors.textMuted }}>
                                            <History size={48} className="mb-3" />
                                            <p className="text-xs font-bold uppercase tracking-[0.2em]">No Activity Logged</p>
                                        </div>
                                    </td>
                                </tr>
                            )}
